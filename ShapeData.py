...
class ShapeDataset(torch.utils.data.Dataset):
    def __init__(self, img_dir):
        self.annotation = torch.zeros(140)
        self.annotation[:70] = 0 #nonsquares are 0
        self.annotation[70:140] = 1 #Squares are 1
    ...
    def __getitem__(self, idx):
        ...
        if label.eq(torch.tensor([1, 0], dtype=torch.int64)).all():
            img_path = os.path.join(self.img_dir, 'pentagon', f"pentagon{idx}.png")

        elif label.eq(torch.tensor([0, 1], dtype=torch.int64)).all():
            img_path = os.path.join(self.img_dir, 'new-square', f"square{idx-70}.png")
